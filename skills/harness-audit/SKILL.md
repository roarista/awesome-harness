...
5. **Evidence drives every proposal — including new mechanisms.** The auditor proposes fixes, and a fix MAY be a *new* mechanism (a hook, gate, skill, routing rule) — NOT only a doc reconciliation. What's banned is the *ungrounded* brainstorm: no proposal without cited evidence of a real problem. Every finding is one of two kinds, and both cite `file:line` / transcript / `settings.json` / command output on the evidence side:
   - **(a) Fidelity drift** — a harness **claim** that contradicts an observed **reality** (both cited). Fix = reconcile (edit the doc, or revive/re-wire the dead mechanism).
   - **(b) Behavioral gap** — recent behavior (last ~5 transcripts) repeatedly does the wrong thing and *no existing mechanism prevents it*. Fix = propose a new mechanism that closes the gap. This is welcome, not out-of-scope — it is the highest-value output. **Canonical example:** the transcripts show the **main session running Edit/Write on source files** across many turns despite the "orchestrator delegates code to codex, never edits directly" policy → propose a **new enforcement hook** (PreToolUse on Write|Edit|MultiEdit) that intercepts a main-session source edit and forces a codex sub-agent spawn using the coding-prompt framework (`/Users/rodrigoarista/Downloads/awesome-harness/docs/CODING_AGENT_PROMPTING.md`). Cite the offending turns as evidence; note whether `route-only-gate.py` already exists but is unarmed for that scope.
   
   The one hard bar: **no fix may cite an imaginary problem.** If you can't point to the drift or the repeated bad behavior in real evidence, it doesn't go in the report. Pure speculation ("might be nice to add X") still goes, capped at 3 bullets, in a fenced `## Out-of-scope observations (speculative)` section at the end — never in the findings table.
...
- **CLAUDE.md line-by-line trim proposal:** `python3 ~/.claude/tools/claudemd-trim.py <repo>` — classifies EVERY substantive line KEEP/TRIM/DELETE/STALE-WRONG with evidence and reports current vs projected byte size. Deterministic stale-path/duplicate stages run with no model; the judgment stage uses `codex exec` over a graphify map. Propose-only: it writes only its report (to `~/engineering-harness/reports/claudemd-trim/`) and never edits a CLAUDE.md; with `--refresh` it also runs `graphify update` in the target repo, so leave that flag off against a live tree. It is also wired to run weekly under launchd `com.ro.engineering-harness-audit`, so a fresh report may already exist (the job is currently UNLOADED pending a Full Disk Access grant — see `tools/launchd/INSTALL-claudemd-trim.md`).
...
- **Findings table** — each: kind (**fidelity-drift** or **behavioral-gap**) → evidence (`file:line`/transcript/output, cited THIS run) → proposed fix → severity (BLOCKER/HIGH/LOW) → confidence. Per rule 5, every row cites real evidence: a fidelity-drift row shows a claim contradicted by reality (fix = reconcile); a behavioral-gap row shows recent transcripts repeatedly doing the wrong thing with no mechanism preventing it (fix = a *new* mechanism — hook/gate/skill/routing rule — that closes the gap). New-mechanism fixes are allowed and encouraged **when grounded in cited bad behavior**; a row whose only basis is "might be nice" is speculation, not a finding — it goes in the fenced Out-of-scope section, not here.
...
- **(Optional) Out-of-scope observations (speculative)** — at most 3 ideas that have NO cited evidence behind them (pure "might be nice"), per rule 5. Grounded new-mechanism proposals do NOT belong here — those are behavioral-gap findings in the table. This section is only for the ungrounded.
