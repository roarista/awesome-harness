...
- **Named creators (harness watch-list)** — recent posts on how they improve their own harnesses/systems:
  - **Boris Cherny**, **Andrej Karpathy**, **Peter Steinberger**, **Jamie West**, **Indie Dev Dan** — + add any new name Ro drops.
  - Sources: their GitHub (repos/gists, sort by updated), personal blog, X/Twitter. **Caveat:** X is hard to read without auth — use web search + `WebFetch` on public post/thread URLs and their blogs; if a post can't be fetched, mark UNKNOWN, don't invent it. For video creators, `youtube-research`/`ytintel`.
- **Reuse `[[deep-research]]`** for the heavy fan-out+adversarial-verify+synthesis rather than hand-rolling searches.
