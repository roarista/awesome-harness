...
2. **Read prior reports before researching.** Read the finding headings from the four most recent `~/Downloads/HARNESS_SCOUT_*.md` reports. Tag every finding `[NEW]` or `[REPEAT n + STATUS]`; a repeat must state in one line whether it was applied, verified against the live tree. Do not re-propose an unlabelled prior finding; prefer new findings.
3. **Fit-filter every idea against OUR constraints — no hype.** We run **API models** (Claude / Codex 5.5) with no weights/residual stream; Ro's machine is **low-CPU, no-VM, no-GPU** (`[[feedback_low_cpu_no_vm]]`); the harness is **ambient hooks + skills**, not a framework Ro invokes (`[[feedback_conversational_prompting]]`); ponytail governs (steal a PATTERN into a few lines, don't add a heavy dep/framework). An idea that needs GPU / open weights / a server / a big runtime is **NON-FIT — say so plainly** and move on. (Canonical NON-FIT: jacobian-lens — white-box, needs CUDA.)
4. **Evidence or it didn't happen (R1).** External claim = **URL + date**, read this run. Repetition finding = **cited transcript turns** (`file:line` of the `.jsonl`). No idea from memory or vibes. Unverifiable → mark UNKNOWN, don't ship it.
5. **Freshness: last ~30 days.** Prefer sources/repos updated in the last 30 days (`gh search repos --sort=updated pushed:>=<date>`; web results dated within a month). Older only if it's canonical/foundational. Note the date on every item so stale intel is obvious.
6. **Bounded fan-out (low-CPU).** At most a **handful** of research sub-agents, not a fleet. Prefer *reusing* existing machinery over standing up new infra: `[[deep-research]]` for the web fan-out+verify+synthesis, `youtube-research`/`ytintel` for creators who post video, `gh` CLI for GitHub. Kill any worker process trees on finish.
7. **Steal the pattern, not the repo.** Default output is "lift this idea into our ambient-hook/skill style in ~N lines", NOT "adopt framework X". Forking a heavy dep is the exception, argued explicitly, never the default.
