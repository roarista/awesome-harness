...
- BUILDER (writes/edits code) = codex 5.5 (spawn the `codex:codex-rescue` subagent). Codex
  carries the coding load (billing). NEVER Claude as the builder — Claude only orchestrates.
- AUDITOR (reviews the build) = Opus 4.8 (low effort) (spawn the `opus` subagent, low effort). A non-builder always checks the builder.
...
PROMPT SHAPE (few points — full guide: /Users/rodrigoarista/Downloads/awesome-harness/docs/CODING_AGENT_PROMPTING.md):
